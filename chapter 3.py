#################################   ПРОВЕРКА ПОСЛЕДОВАТЕЛЬНОСТИ МНОЖЕСТВЕННОГО НАСЛЕДОВАНИЯ    #######################
class BaseClass:
    num_base_calls = 0
    def call_me(self):
...
class Property:
    def __init__(self, square_feet='', beds='', baths='', **kwargs): #  *kwargs добавлен, потому что ожидается
...
class Purchase:
    ...
    def display(self):
        super().display()  #  Этот метод сработает при множественном наследовании, если хоть в одном из классов из
        print("PURCHASE DETAILS") #  сиска множественного наследования есть родитель, у которого существует этот
        print("selling price: {}".format(self.price)) #  метод!!! Не забывать, что super() работает как NEXT!!!!
...
class Rental:
    ...
    @staticmethod
    def prompt_init():
        return dict(rent=input("What is the monthly rent? "), utilities=input(
